              ...
              <el-row type="flex"
                ...
                <el-col :span="6">
                  <el-button type="primary"
                             size="small"
                             class="text-right"
                             @click="updateTodoList(item)">已完成</el-button>
              ...
              <el-row type="flex"
                ...
                <el-col :span="4">
                  <el-button type="warning"
                             size="small"
                             class="text-right"
                             @click="updateTodoList(item, 'completeList')">撤销</el-button>
                </el-col>
              </el-row>
            </template>
            <template v-else>
...
export default {
  ...
  methods: {
    ...
    remove (item) {
      ...
      this.$http
        .post("/api/user/DestroyTodoList", data)
        .then(res => {
          if (res.data.code === 1) {
            const list = this.listArr.todoList;
            this.$message({
    ...
    updateTodoList (item, listName = "todoList") {
      ...
      this.$http
        .post("/api/user/updateTodoList", data)
        .then(res => {
          if (res.data.code === 1) {
    ...
    getTodoList (list) {
      this.$http
        .get(
          "/api/user/getTodoList?status=" +
          list.status +
          "&user_id=" +
          JSON.parse(localStorage.getItem("todolist")).id
        )
        .then(res => {
          if (res.data.code === 1) {
