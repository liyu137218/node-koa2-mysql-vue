  ...
  <div>
    <el-row type="flex" justify="center" class="mb-20">
      <el-col :span="6" class="display-block">
        {{user.name}} 客官，您有什么吩咐
        <el-button type="info" @click="exit" size="small" class="ml-20">退出</el-button>
      </el-col>
    </el-row>
    <el-row type="flex" justify="center" class="mb-20">
      <el-col :span="6">
        ...
        <el-tabs v-model="activeName" @tab-click="switchClick">
          <el-tab-pane name="todoList" label="待办事项">
            <template v-if="todoList.length !== 0">
              ...
              >
                ...
                <el-col class="text-justify" :span="16">{{item.content}}</el-col>
                <el-col :span="6">
                  <el-button
                    type="primary"
                    size="small"
                    class="text-right"
                    @click="updateTodoList(item)"
                  >已完成</el-button>
          ...
          <el-tab-pane name="completeList" label="已完成事项">
            <template v-if="completeList.length !== 0">
              ...
              >
                <el-col :span="2">{{index + 1}}</el-col>
                <el-col class="text-justify" :span="18">{{item.content}}</el-col>
                <el-col :span="4">
                  <el-button
                    type="warning"
                    size="small"
                    class="text-right"
                    @click="updateTodoList(item, 'completeList')"
                  >撤销</el-button>
                </el-col>
              </el-row>
            </template>
...
export default {
  ...
  methods: {
     ...
     */
    getTodoList(list) {
      this.$http.get('/api/user/getTodoList?status=' + list.status + '&user_id=' + JSON.parse(localStorage.getItem('todolist')).id)
        .then((res) => {
          if (res.data.code === 1) {
            this.todoList = res.data.bean.list
